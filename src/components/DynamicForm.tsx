...
export const DynamicForm: React.FC<DynamicFormProps> = ({ entity }) => {
    ...
    const [sections, setSections] = useState<any>(['General']);
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            console.log(`DynamicForm: Loading schema:`, schema);


            if (schema && schema.length > 0 && entity) {
                const foundTable = schema.find((s: any) => s.entity === entity);
                setConfig(foundTable);
                setFormConfig(foundTable?.form || []);
                console.log("DynamicForm: Setting up defaults from formConfig:q", foundTable?.form);
            }
    ...
    const mainSections = formConfig.filter(s => s.id !== 'sidebar');
    const sidebarSections = formConfig.filter(s => s.id === 'sidebar');
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-slate-900">
                            {id ? t('common.edit') : t('common.create')} {config?.labels?.singular || entity}
                        </h1>
