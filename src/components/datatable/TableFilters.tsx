...
export const TableFilters: React.FC<TableFiltersProps> = ({
    ...
    if(!filtersConfig || (!filtersConfig.search_input && !filtersConfig.columns && !filtersConfig.more_enabled)){
        return null;
    }
                ...
                {showFilters && (
                    ...
                    >
                        <div className="flex flex-wrap gap-4 p-4 bg-slate-50/50 rounded-2xl border border-slate-100">
                            {filtersConfig?.more ? (
                                filtersConfig.more.map(filter => renderFilterInput(filter))
                            ) : (
                                <>
                                    <div className="flex items-center gap-2">
